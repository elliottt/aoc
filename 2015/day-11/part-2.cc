...
bool increasing_straight(const std::string &password) {
...
bool no_easily_mistaken(const std::string &password) {
    return !ranges::any_of(password, [](auto c) { return c == 'i' || c == 'o' || c == 'l'; });
}

bool double_letters(const std::string &password) {
...
std::string next_password(const std::string &password) {
