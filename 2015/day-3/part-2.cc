...
auto total_visited(string directions) {
    ...
    return ranges::accumulate(all_houses
        | views::group_by(&point::operator==)
        | views::transform([](auto group) { return 1; }), 0);
}
